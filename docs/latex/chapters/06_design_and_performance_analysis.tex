...
\subsubsubsection{Sample Case Illustration}
...
Figure \ref{fig:mtr_prediction_merged} demonstrates MTR's multimodal prediction capabilities in a right-turn scenario. The model generates multiple future trajectories with assigned probabilities for each mode. The green line represents the ground truth future path, while the colored lines show the model's predictions. The high-probability prediction (p=0.36) closely follows the ground truth trajectory, while the low-probability alternative (p=0.02) captures a different plausible behavior mode. This example illustrates how MTR captures the inherent uncertainty in agent behavior while providing interpretable confidence estimates for each predicted mode.
