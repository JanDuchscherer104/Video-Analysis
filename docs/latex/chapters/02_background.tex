...
\begin{description}[style=nextline,leftmargin=*]
  \item[Agent states.]
        ...
        \end{equation}
  \item[Map polylines.]
        Lanes are stored either as motion vectors or segment lists. Each segment's start point acts as the origin and the segment vector defines the \(x\)-axis, leaving only the (normalized) length as an explicit feature.
\end{description}

\paragraph{Relative descriptors and embeddings.}
For any pair \((e_i,t)\), \((e_j,\tau)\) of scene elements, the query-centric paradigm computes a 4-D descriptor that fully preserves their spatial-temporal offset:
\begin{equation}
r_{j\to i}^{\tau\to t}=
\bigl[
    \|p_j^\tau-p_i^t\|_2,\;
    \mathrm{atan2}(p_{j,y}^\tau-p_{i,y}^t,p_{j,x}^\tau-p_{i,x}^t)-\theta_i^t,\;
    \theta_j^\tau-\theta_i^t,\;
    \tau-t
\bigr].
\label{eq:relative_descriptor}
\end{equation}
Each component encodes a crucial geometric relationship:
\begin{enumerate}[label=(\roman*)]
    \item The Euclidean distance \(\|p_j^\tau-p_i^t\|_2\) captures the spatial separation between elements.
...
Consequently, we hypothesize that the model can split its representational capacity to learning the \emph{relations between fibres}, as well as the relatively \emph{simple structures of the fibers} themselves instead of having to learn a non-decomposable global scene manifold.
